...
// Read from disk rather than a hand-kept list, because every upstream bump adds plugin-sdk modules and a stale list makes each new one fail to resolve at test time.
const pluginSdkSubpaths = fs
  .readdirSync(path.join(repoRoot, "src", "plugin-sdk"))
  .filter((entry) => entry.endsWith(".ts") && !entry.endsWith(".test.ts"))
  .map((entry) => entry.slice(0, -".ts".length))
  .filter((subpath) => subpath !== "index");
